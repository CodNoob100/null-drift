...
app = FastAPI(
    title="null-drift-gateway", description="ML Inference Gateway for the HRSA daemon"
)
...
class TextRequest(BaseModel):
    ...
    salience: float = Field(
        ..., ge=0.0, le=1.0, description="Salience weight for the memory (0.0 to 1.0)"
    )
...
async def snapshot_state(thread_id: str = Query("default")):
    ...
    try:
        response = requests.post(
            f"{NULLD_URL}/snapshot", params={"thread_id": thread_id}, timeout=5.0
        )
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
...
async def restore_state(thread_id: str = Query("default")):
    ...
    try:
        response = requests.post(
            f"{NULLD_URL}/restore", params={"thread_id": thread_id}, timeout=5.0
        )
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
