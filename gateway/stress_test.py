...
class StressTest:
    def __init__(self, target_events=10000):
        ...
        self.milestone_indices = sorted(
            random.sample(range(100, target_events - 100), len(CAUSAL_MILESTONES))
        )
        self.session = None
    ...
    async def run(self):
        ...
        for i in range(self.target_events):
            if (
                milestone_counter < len(self.milestone_indices)
                and i == self.milestone_indices[milestone_counter]
            ):
                ...
                tasks.append(
                    asyncio.create_task(
                        self.inject(text, salience, is_milestone=False, step=i)
                    )
                )
    ...
    async def inject(self, text, salience, is_milestone, step):
        ...
        try:
            async with self.session.post(
                f"{self.base_url}/inject", json=payload
            ) as response:
                await response.json()
                if is_milestone:
                    print(
                        f"  [MILESTONE] INJECTED: '{text}' (Step {step}, Salience: {salience:.2f})"
                    )
                elif step % 1000 == 0:
