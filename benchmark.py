...
def run_benchmark(num_requests=100):
    ...
    for i in range(num_requests):
        ...
        res = requests.post(f"{GATEWAY_URL}/inject?thread_id={THREAD_ID}", json={
            "text": f"This is test memory iteration {i} to benchmark the HRSA hyperdimensional vector projection speed.",
            "salience": 0.8
        })
    ...
    print(f"Average Inject Latency (End-to-End): {statistics.mean(inject_times):.2f} ms")
